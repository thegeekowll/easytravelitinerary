...
export default function ItineraryDetailPage({
  ...
  const { data: destinationsData } = useQuery({
    ...
    queryFn: () => apiClient.getDestinations({ limit: 100 }),
  });
  const { data: accommodationsData } = useQuery({
    queryKey: ["accommodations", "all"],
    queryFn: () => apiClient.getAccommodations({ limit: 100 }),
  });
  const { data: tourTypes } = useQuery({
    queryKey: ["tourTypes"],
    queryFn: () => apiClient.getTourTypes(),
  });
  const { data: accLevels } = useQuery({
    queryKey: ["accLevels"],
    queryFn: () => apiClient.getAccommodationLevels(),
  });
  ...
  useEffect(() => {
    ...
    if (initialItinerary && !itinerary) {
      ...
      const mappedDays = (initialItinerary.days || []).map((day: any) => ({
        ...day,
        destination_ids:
          day.destination_ids || (day.destinations || []).map((d: any) => d.id),
      }));
  ...
  useEffect(() => {
    if (isEditing && itinerary) {
      ...
      if (deposit > 0) {
        newStatus =
          deposit >= total && total > 0 ? "fully_paid" : "partially_paid";
      }

      if (itinerary.payment_status !== newStatus) {
        setItinerary((prev: any) => ({ ...prev, payment_status: newStatus }));
      }
    }
  ...
  const handleDayChange = (updatedDay: any) => {
    const newDays = itinerary.days.map((d: any) =>
      d.day_number === updatedDay.day_number ? updatedDay : d,
    );
  ...
  const handleRemoveDay = (dayNum: number) => {
    ...
    const newDays = itinerary.days
      .filter((d: any) => d.day_number !== dayNum)
      .map((d: any, idx: number) => ({ ...d, day_number: idx + 1 }));
    setItinerary({ ...itinerary, days: newDays });
  };

  // Show loader if:
  // 1. React Query is loading
  // 2. React Query has data (initialItinerary) but local state (itinerary) hasn't synced yet
  if (isLoading || (initialItinerary && !itinerary)) {
    return (
  ...
  // Show error if:
  // 1. React Query returned an error
  // 2. React Query finished loading, has no data, and we have no local state
  if (error || (!isLoading && !initialItinerary && !itinerary)) {
    return (
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            <Button
              ...
              size="sm"
              onClick={() => router.back()}
              className="p-0 h-auto hover:bg-transparent text-gray-500"
            >
          ...
          <div className="flex items-center gap-3">
            ...
            <Badge variant={statusColors[itinerary.status] as any}>
              {itinerary.status}
            </Badge>
          </div>
            ...
            <CardContent className="space-y-4">
                ...
                {isEditing ? (
                  <select
                    ...
                    onChange={(e) =>
                      setItinerary({
                        ...itinerary,
                        tour_type_id: e.target.value,
                      })
                    }
                  >
                    <option value="">Select Type</option>
                    {(Array.isArray(tourTypes)
                      ? tourTypes
                      : tourTypes?.items || []
                    )?.map((t: any) => (
                  ...
                  <p className="font-medium">
                    {itinerary.tour_type?.name || "Unspecified"}
                  </p>
                )}
              </div>
                ...
                {isEditing ? (
                  <select
                    ...
                    onChange={(e) =>
                      setItinerary({
                        ...itinerary,
                        accommodation_level_id: e.target.value,
                      })
                    }
                  >
                    <option value="">Select Level</option>
                    {(Array.isArray(accLevels)
                      ? accLevels
                      : accLevels?.items || []
                    )?.map((l: any) => (
                  ...
                  <p className="font-medium">
                    {itinerary.accommodation_level?.name ||
                      (accLevels?.map
                        ? accLevels.find(
                            (l: any) =>
                              l.id === itinerary.accommodation_level_id,
                          )?.name
                        : accLevels?.items?.find(
                            (l: any) =>
                              l.id === itinerary.accommodation_level_id,
                          )?.name || "Unspecified")}
                  </p>
              ...
              <div className="space-y-1">
                ...
                {isEditing ? (
                  ...
                  <p className="font-medium">
                    {itinerary.difficulty_level || "Unspecified"}
                  </p>
                )}
              </div>
            </CardContent>
                ...
                <div className="col-span-2 flex flex-col gap-2">
                  ...
                  <div className="w-full">
                    <Badge
                      ...
                      variant={
                        itinerary.payment_status === "fully_paid"
                          ? "success"
                ...
                <div className="space-y-1">
                  ...
                  {isEditing ? (
                    <div className="relative">
                      <span className="absolute left-2 top-2.5 text-gray-500 text-xs">
                        $
                      </span>
                      <Input
                        className="pl-6"
                        type="number"
                        value={itinerary.total_price}
                        onChange={(e) =>
                          setItinerary({
                            ...itinerary,
                    ...
                    <p className="font-bold text-lg">
                      {itinerary.currency} {itinerary.total_price}
                    </p>
                  )}
                </div>
                  ...
                  {isEditing ? (
                    <div className="relative">
                      ...
                      <Input
                        className="pl-6"
                        type="number"
                        value={itinerary.deposit_amount}
                        onChange={(e) =>
                          setItinerary({
                            ...itinerary,
                    ...
                    <p className="font-medium text-gray-600">
                      {itinerary.currency} {itinerary.deposit_amount}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                {isEditing ? (
                  <select
                    ...
                    onChange={(e) =>
                      setItinerary({ ...itinerary, status: e.target.value })
                    }
                  >
              ...
              <div className="space-y-1">
                ...
                {isEditing ? (
                  <textarea
                    ...
                    onChange={(e) =>
                      setItinerary({ ...itinerary, notes: e.target.value })
                    }
                    placeholder="Internal notes not visible to customer..."
                  />
                ) : (
                  <p className="text-sm text-gray-600 whitespace-pre-line">
                    {itinerary.notes || "No notes"}
                  </p>
                )}
              </div>
            </CardContent>
                ...
                <>
                  <div className="space-y-2">
                    ...
                    <InclusionManager
                      type="inclusion"
                      selectedIds={
                        itinerary.inclusions?.map((i: any) => i.id) || []
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <InclusionManager
                      type="exclusion"
                      selectedIds={
                        itinerary.exclusions?.map((e: any) => e.id) || []
                      }
                      onSelectionChange={(_ids, items) =>
                        setItinerary({ ...itinerary, exclusions: items || [] })
                      }
                    />
                  </div>
                </>
                ...
                <div className="space-y-4 text-sm">
                  <div>
                    ...
                    <ul className="list-disc pl-4 space-y-1">
                      {itinerary.inclusions?.map((i: any) => (
                  ...
                  <div>
                    ...
                    <ul className="list-disc pl-4 space-y-1">
                      {itinerary.exclusions?.map((e: any) => (
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                {isEditing ? (
                  <Input
                    value={itinerary.tour_title}
                    onChange={(e) =>
                      setItinerary({ ...itinerary, tour_title: e.target.value })
                    }
                  />
                ) : (
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <p>{new Date(itinerary.return_date).toLocaleDateString()}</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                {isEditing ? (
                  <textarea
                    ...
                    onChange={(e) =>
                      setItinerary({ ...itinerary, highlights: e.target.value })
                    }
                  />
                ) : (
                  <p className="text-gray-700 whitespace-pre-line">
                    {itinerary.highlights || "No highlights listed."}
                  </p>
                )}
              </div>
            </CardContent>
              ...
              {isEditing ? (
                  ...
                  {[
                    {
                      role: "cover",
                      label: "Cover Photo",
                      desc: "Main image displayed at the top",
                    },
                    {
                      role: "accommodation_end",
                      label: "After Accommodations",
                      desc: "Displayed after the accommodation section",
                    },
                    {
                      role: "inclusions",
                      label: "Includes/Excludes Page",
                      desc: "Background for inclusions section",
                    },
                    {
                      role: "about_banner",
                      label: "About Us Banner",
                      desc: "Banner for the company info section",
                    },
                    {
                      role: "end",
                      label: "End of Itinerary",
                      desc: "Final image at the end of the document",
                    },
                  ].map((slot) => {
                    const activeImage = itinerary.images?.find(
                      (img: any) => img.image_role === slot.role,
                    );
                          ...
                          <div className="relative aspect-video w-full max-w-md rounded-md overflow-hidden group">
                            ...
                            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                              <Button
                                ...
                                onClick={async () => {
                                  try {
                                    await apiClient.deleteItineraryImage(
                                      activeImage.id,
                                    );
                                    setItinerary((prev: any) => ({
                                      ...prev,
                                      images: prev.images.filter(
                                        (img: any) => img.id !== activeImage.id,
                                      ),
                                    }));
                              ...
                              <input
                                ...
                                onChange={async (e) => {
                                  ...
                                  try {
                                    // Upload
                                    const newImages =
                                      await apiClient.uploadItineraryImages(
                                        itinerary.id,
                                        [file],
                                      );
                                    ...
                                    // Update Role
                                    await apiClient.updateItineraryImage(
                                      newImage.id,
                                      { image_role: slot.role },
                                    );
                  ...
                  <ImageGalleryModal
                    ...
                    onSelect={async (selectedImages) => {
                      ...
                      try {
                        ...
                        const linkedImages =
                          await apiClient.linkItineraryImages(itinerary.id, [
                            {
                              image_url: img.url,
                              caption: img.caption,
                            },
                        ...
                        setItinerary((prev: any) => ({
                          ...
                          images: [
                            ...(prev.images || []).filter(
                              (i: any) => i.image_role !== targetImageRole,
                            ),
                            {
                              ...newItineraryImage,
                              image_role: targetImageRole,
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {[
                    ...
                    const img = itinerary.images?.find(
                      (i: any) => i.image_role === slot.role,
                    );
                    if (!img) return null;
                    return (
                      <div
            ...
            <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600 bg-gray-50 p-4 rounded-lg border border-gray-100">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              {(typeof day.accommodation === "object"
                                ...
                                : day.accommodation) || "Not selected"}
                            </div>
                          </div>
                          ...
                          {day.activities && (
                            <div className="md:col-span-2 pt-2 border-t mt-2">
                              <span className="font-semibold block mb-1 text-xs uppercase text-gray-500">
                                Activities
                              </span>
                              <p>{day.activities}</p>
                            </div>
              ...
              {(!itinerary.days || itinerary.days.length === 0) && (
                <div className="text-center py-10 border-2 border-dashed rounded-lg text-gray-400">
                  ...
                  {isEditing && (
                    <Button variant="link" onClick={handleAddDay}>
                      Add the first day
                    </Button>
                  )}
                </div>
